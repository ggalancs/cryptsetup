...
static bool json_segment_contains_flag(json_object *jobj_segment, const char *prefix, size_t prefix_len)
{
	int i, r;
	...
	for (i = 0; i < (int)json_object_array_length(jobj_flags); i++) {
		...
		r = prefix ? strncmp(json_object_get_string(jobj), prefix, prefix_len) :
			     strcmp(json_object_get_string(jobj), prefix);
		if (!r)
			return true;
	}
