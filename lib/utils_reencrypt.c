	...
	struct crypt_params_luks2 *params) /* NULL if not changed */
{
	...
	data_offset = LUKS2_get_data_offset(hdr);
	move_first_segment = (!strcmp(reencrypt_mode, "encrypt") && data_offset != 0 && data_shift < 0);
	...
	if (move_first_segment) {
		...
		if (imaxabs(data_shift) < LUKS2_get_data_offset(hdr)) {
			log_err(cd, "Data shift (%" PRIu64 " sectors) is less than future data offset (%" PRIu64 " sectors).", imaxabs(data_shift), LUKS2_get_data_offset(hdr));
			r = -EINVAL;
			goto err;
		}
		devfd = device_open_excl(cd, crypt_data_device(cd), O_RDWR);
...
out:
	...
	return r < 0 ? r : reencrypt_keyslot;
}
